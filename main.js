const PageFlip =
  window && window.St && window.St.PageFlip
    ? window.St.PageFlip
    : window && window.PageFlip
      ? window.PageFlip
      : null;
if (!PageFlip) {
  throw new Error(
    "PageFlip library not found. Check index.html includes page-flip browser script.",
  );
}

// Ambient audio (WebAudio) — declared early to avoid TDZ errors in some runtimes (e.g., Replit).
let ambientCtx = null;
let ambientNode = null;

// This project runs in both GitHub Pages (built) and Replit (Vite dev).
// To keep interactions (especially "Click to Open") reliable across hosts,
// we attach DOM listeners after DOMContentLoaded.
...
function runApp() {
  ...
  function unlockAudioOnce() {
    ...
    try {
      if (ambientCtx && typeof ambientCtx.resume === "function")
        ambientCtx.resume();
  ...
  const UI = {
    ...
    chaptersDrawer: document.getElementById("chapters-drawer"),
    chaptersList: document.getElementById("chapters-list"),
    chapterLabel: document.getElementById("ui-chapter"),
    pageLabel: document.getElementById("ui-page"),
    ...
    egg: document.getElementById("easter-egg"),
    eggClose: document.getElementById("btn-egg-close"),
  };

  const STORAGE_KEYS = {
    hintSeen: "chapter21_hint_seen_v1",
    soundEnabled: "chapter21_sound_enabled_v1",
  };

  // Sound toggle (OFF by default)
  let soundEnabled = localStorage.getItem(STORAGE_KEYS.soundEnabled) === "1";
  function setSoundEnabled(next) {
    soundEnabled = Boolean(next);
    localStorage.setItem(STORAGE_KEYS.soundEnabled, soundEnabled ? "1" : "0");
    if (UI.soundBtn) UI.soundBtn.textContent = soundEnabled ? "🔊" : "🔇";
    // Ambient follows the same toggle
  ...
  // Ambient (WebAudio, lightweight, no external file)
  ...
  function stopAmbient() {
    try {
      if (ambientNode) {
        // If we stored nodes in an object, disconnect safely.
        if (ambientNode.stop) ambientNode.stop(0);
        if (ambientNode.source && ambientNode.source.stop)
          ambientNode.source.stop(0);
        if (ambientNode.disconnect) ambientNode.disconnect();
        if (ambientNode.source && ambientNode.source.disconnect)
          ambientNode.source.disconnect();
        ambientNode = null;
      }
      if (ambientCtx) {
        if (ambientCtx.close) ambientCtx.close();
        ambientCtx = null;
      }
    } catch (e) {
  ...
  // Drawer menu
  function openDrawer() {
    if (!UI.chaptersDrawer) return;
    UI.chaptersDrawer.classList.remove("hidden");
    UI.chaptersDrawer.setAttribute("aria-hidden", "false");
  }
  function closeDrawer() {
    if (!UI.chaptersDrawer) return;
    UI.chaptersDrawer.classList.add("hidden");
    UI.chaptersDrawer.setAttribute("aria-hidden", "true");
  }
  if (UI.menuBtn)
  ...
  // Easter egg: tap the heart 5 times on cover
  let heartTaps = 0;
  function setupEasterEgg() {
    const heart = document.querySelector(".heart-decoration");
    if (!heart || !UI.egg || !UI.eggClose) return;
    heart.style.cursor = "pointer";
    heart.addEventListener("click", () => {
      heartTaps += 1;
      if (heartTaps >= 5) {
        UI.egg.classList.remove("hidden");
        heartTaps = 0;
      }
    });
    UI.eggClose.addEventListener("click", () => UI.egg.classList.add("hidden"));
  }
  setupEasterEgg();
  ...
  function updateProgressUI() {
    ...
    try {
      const total =
        pageFlipInstance && typeof pageFlipInstance.getPageCount === "function"
      ...
      // Find current chapter by nearest previous marker
      const markers = Array.from(
        document.querySelectorAll("[data-chapter-index]"),
      )
        .map((el) => ({
          idx: Number(el.getAttribute("data-page-index") || 0),
          title: el.getAttribute("data-chapter-title") || "Chapter",
        }))
        .sort((a, b) => a.idx - b.idx);

      let chapterTitle = "Chapter";
      for (const m of markers) {
        if (m.idx <= idx) chapterTitle = m.title;
        else break;
      }

      if (UI.chapterLabel) UI.chapterLabel.textContent = chapterTitle;
      if (UI.pageLabel) UI.pageLabel.textContent = `Page ${current} / ${total}`;
      if (UI.progressFill && total > 0)
        UI.progressFill.style.width = `${(current / total) * 100}%`;
    } catch (_) {}
  }

  function buildChaptersDrawer() {
    ...
    const markers = Array.from(
      ...
      .map((el) => ({
        pageIndex: Number(el.getAttribute("data-page-index") || 0),
        title: el.getAttribute("data-chapter-title") || "Chapter",
      }))
      ...
      btn.addEventListener("click", () => {
        ...
        try {
          if (
            ...
            pageFlipInstance.turnToPage(m.pageIndex);
        } catch (_) {}
      });
      UI.chaptersList.appendChild(btn);
    });
  }
  ...
  `;
    ...
    bookContent.chapters.forEach((chapter, chapterIndex) => {
      ...
      while (i < chapter.items.length) {
        ...
        // Mark the first page of each chapter for the chapters drawer
        if (i === 0) {
          const nextIndex = bookElement.children.length;
          page.setAttribute("data-chapter-index", String(chapterIndex));
          page.setAttribute("data-chapter-title", chapter.title);
          page.setAttribute("data-page-index", String(nextIndex));
        }
        // Base decorations
        let content = `
        ${getRandomStains()}
  ...
  if (!coverEl) {
    console.error(
      "Cover container (#book-cover-container) not found. Click-to-open disabled.",
    );
    return;
  }
  ...
  coverEl.addEventListener("click", () => {
    if (isOnBackCover) return;
    unlockAudioOnce();
    // Ensure bookmark ribbon exists
    const bc = document.getElementById("book-container");
    if (bc && !bc.querySelector(".bookmark-ribbon")) {
      const r = document.createElement("div");
      r.className = "bookmark-ribbon";
      bc.appendChild(r);
    }
  ...
  function onSwipeStart(e) {
    // Only track swipes when the cover is visible (front or back)
    const coverContainer = document.getElementById("book-cover-container");
    if (!coverContainer || coverContainer.style.display === "none") return;
  ...
  function onSwipeEnd(e) {
    ...
    // Only act when the cover is visible
    if (!coverContainer || coverContainer.style.display === "none") return;
    ...
    // Require a clear horizontal swipe; ignore mostly-vertical gestures
    if (Math.abs(dx) < 70 || Math.abs(dx) < Math.abs(dy)) return;

    // 👉 BACK COVER → swipe RIGHT → reopen from end
    if (isOnBackCover && dx > 0) {
      openFromBackCover();
      return;
    }

    // 👉 FRONT COVER → swipe LEFT → open from start
    if (!isOnBackCover && dx < 0) {
      unlockAudioOnce();
